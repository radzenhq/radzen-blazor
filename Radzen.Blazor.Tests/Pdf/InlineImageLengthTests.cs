...
public class InlineImageLengthTests
{
    ...
    public void InlineImagePayloadContainingBoundedEI_DoesNotTruncateEarly()
    {
        ...
        bytes.AddRange(Latin1("q BI /W 4 /H 1 /CS /G /BPC 8 ID "));
        bytes.AddRange(new byte[] { (byte)'E', (byte)'I', (byte)' ', 0x00 });
        bytes.AddRange(Latin1(" EI\n10 20 m Q"));
    ...
    {
        var bytes = new List<byte>();
        bytes.AddRange(Latin1("BI /W 2 /H 2 /CS /RGB /BPC 8 /F /AHx /L 5 ID "));
    ...
    {
        var bytes = new List<byte>();
        bytes.AddRange(Latin1("BI /W 2 /H 2 /F /Fl ID "));
