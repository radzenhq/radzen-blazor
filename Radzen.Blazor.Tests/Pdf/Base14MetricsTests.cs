...
public class Base14MetricsTests
{
    ...
    public void MeasureString_Latin1SampleEqualsAfmWidthSum()
    {
        ...
        var metrics = Base14Metrics.Resolve(MakeFont("Times-Roman"));
        Assert.NotNull(metrics);
    ...
    [Fact]
    public void GetWidth_KnownCodeMatchesAfm()
    {
        var afm = AfmReference.Load("Helvetica");
        var metrics = Base14Metrics.Resolve(MakeFont("Helvetica"))!;

        Assert.True(WinAnsiEncoding.TryGetCode('A', out var codeA));
        Assert.Equal((double)afm.WidthByName["A"], metrics.GetWidth(codeA));

        Assert.True(WinAnsiEncoding.TryGetCode('é', out var codeE));
        Assert.Equal((double)afm.WidthByName["eacute"], metrics.GetWidth(codeE));
    }

    [Theory]
    [InlineData(129)]
    [InlineData(0)]
    public void GetWidth_UndefinedCodeIsZero(int code)
    {
        var metrics = Base14Metrics.Resolve(MakeFont("Helvetica"))!;
        Assert.Equal(0d, metrics.GetWidth((byte)code));
    }
    ...
    {
        var metrics = Base14Metrics.Resolve(MakeFont(family, bold, italic));
        Assert.NotNull(metrics);
    ...
    {
        var metrics = Base14Metrics.Resolve(MakeFont("helvetica", bold: true));
        Assert.NotNull(metrics);
    ...
    {
        Assert.Null(Base14Metrics.Resolve(MakeFont(name)));
    }
    ...
    {
        var metrics = Base14Metrics.Resolve(MakeFont(family, bold: true, italic: true));
        Assert.NotNull(metrics);
        Assert.Equal(expected, metrics!.PostScriptName);
    }
    ...
    public void GetWidth_SymbolicFontsUseNativeEncoding(string family, int code)
    {
        ...
        var metrics = Base14Metrics.Resolve(MakeFont(family))!;
        Assert.Equal((double)afm.WidthByCode[code], metrics.GetWidth((byte)code));
    }
    ...
    {
        var afm = AfmReference.Load(afmFile);
        var metrics = Base14Metrics.Resolve(MakeFont(afmFile))!;
    ...
    public void ItalicAngle_IsNonZeroForObliqueVariant()
    {
        ...
        var metrics = Base14Metrics.Resolve(MakeFont("Helvetica", italic: true))!;
        Assert.Equal(afm.ItalicAngle, metrics.ItalicAngle);
    ...
    [InlineData('é', true)]
    [InlineData('Б', false)]
    public void ContainsGlyph_ReflectsWinAnsiCoverage(char c, bool expected)
    {
        var metrics = Base14Metrics.Resolve(MakeFont("Helvetica"))!;
