...
public class WorkbookStreamedRowsTests
{
    ...
    private static async IAsyncEnumerable<(CellData? Data, Format? Format)[]> FormattedRows(params (CellData? Data, Format? Format)[][] rows)
    {
        foreach (var row in rows)
        {
            await Task.Yield();
    ...
    [InlineData(false)]
    [InlineData(true)]
    public async Task Formatted_rows_preserve_values_and_custom_formats(bool inline)
    {
        var currency = new Format { NumberFormat = "$#,##0.0000" };
        var date = new Format { NumberFormat = "yyyy-mm-dd" };
        var blank = new Format { BackgroundColor = "#FF0000", Bold = true };
        using var stream = new MemoryStream();
        await Framed().SaveToStreamAsync(stream, FormattedRows(
            [(CellData.FromNumber(12.3456), currency), (CellData.FromDate(Seed), date),
             (CellData.FromString("00123"), currency), (null, blank), (null, null),
             (CellData.FromBoolean(true), null), (CellData.FromDate(Seed), null)]), inline);

        var sheet = Read(stream);
        Assert.Equal(12.3456, sheet.Cells[1, 0].Value);
        Assert.Equal(currency.NumberFormat, sheet.Cells[1, 0].Format.NumberFormat);
        Assert.Equal(Seed.ToOADate(), sheet.Cells[1, 1].Value);
        Assert.Equal(date.NumberFormat, sheet.Cells[1, 1].Format.NumberFormat);
        Assert.Equal("00123", sheet.Cells[1, 2].Value);
        Assert.Null(sheet.Cells[1, 3].Value);
        Assert.True(sheet.Cells[1, 3].Format.Bold);
        Assert.Equal(true, sheet.Cells[1, 5].Value);
        Assert.Equal(Seed.ToOADate(), sheet.Cells[1, 6].Value);
        var cells = Row(stream, 2).Elements(Main + "c").ToArray();
        Assert.DoesNotContain(cells, c => (string?)c.Attribute("r") == "E2");
        Assert.Equal(inline ? "inlineStr" : "s", (string?)cells[2].Attribute("t"));
    }
    ...
    public async Task Reused_format_distinguishes_value_type_and_quoting()
    {
        ...
        {
            (CellData.FromNumber(42), format), (CellData.FromDate(Seed), format),
            (CellData.FromString("00123"), format), (CellData.FromString("plain"), format)
        };
        using var stream = new MemoryStream();
        await Framed().SaveToStreamAsync(stream, FormattedRows(Enumerable.Repeat(row, 1000).ToArray()));
        var first = Row(stream, 2).Elements(Main + "c").Select(c => (string?)c.Attribute("s")).ToArray();
        var last = Row(stream, 1001).Elements(Main + "c").Select(c => (string?)c.Attribute("s")).ToArray();
    ...
    {
        public override bool Equals(object? obj) => obj is EqualFormat;
        public override int GetHashCode() => 0;
    }

    [Fact]
    public async Task Cache_uses_format_identity_even_when_equality_is_overridden()
    {
        using var stream = new MemoryStream();
        await Framed().SaveToStreamAsync(stream, FormattedRows(
            [(CellData.FromNumber(1), new EqualFormat { NumberFormat = "0.0000" })],
            [(CellData.FromNumber(2), new EqualFormat { NumberFormat = "0%" })]));
        var sheet = Read(stream);
        Assert.Equal("0.0000", sheet.Cells[1, 0].Format.NumberFormat);
        Assert.Equal("0%", sheet.Cells[2, 0].Format.NumberFormat);
    }

    [Fact]
    public async Task Formatted_source_failure_restores_output()
    {
        static async IAsyncEnumerable<(CellData? Data, Format? Format)[]> Failing()
        {
            yield return [(CellData.FromNumber(1), new Format { Bold = true })];
            await Task.Yield();
        ...
        using var stream = new MemoryStream();
        stream.WriteByte(42);
        await Assert.ThrowsAsync<InvalidOperationException>(() => Framed().SaveToStreamAsync(stream, Failing()));
