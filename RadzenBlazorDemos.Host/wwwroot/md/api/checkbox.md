# RadzenCheckBox API Reference

## Parameters

| Parameter | Type | Description |
|-----------|------|-------------|
| AriaLabel | `string?` | Gets or sets the aria-label attribute of the checkbox. |
...
| ReadOnly | `bool` | Gets or sets whether the checkbox is read-only and cannot be toggled by user interaction. When true, the checkbox displays its current state but prevents clicking or keyboard toggling. Useful for displaying checkbox state in view-only scenarios. |
...
| TriState | `bool` | Gets or sets whether the checkbox supports three states: checked (true), unchecked (false), and indeterminate (null). When enabled, clicking cycles through all three states. Use with nullable boolean (bool?) values. |
