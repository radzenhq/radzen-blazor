# RadzenCheckBoxList API Reference
...
| AllowSelectAll | `bool` | Gets or sets a value indicating whether the user can select all values. Set to false by default. |
| Attributes | `IReadOnlyDictionary<string, object>?` | Gets or sets a dictionary of additional HTML attributes that will be applied to the component's root element. Any attributes not explicitly defined as parameters will be captured here and rendered on the element. Use this to add data-* attributes, ARIA attributes, or any custom HTML attributes. |
| Culture | `CultureInfo` | Gets or sets the culture used for formatting and parsing localizable data (numbers, dates, currency). If not set, uses the from a parent component or falls back to . |
| Data | `IEnumerable?` | Gets or sets the data used to generate items. |
...
| Gap | `string?` | Gets or sets the spacing between items |
| Items | `RenderFragment?` | Gets or sets the items that will be concatenated with generated items from Data. |
...
| ReadOnlyProperty | `string?` | Gets or sets the read-only property. |
...
| Wrap | `FlexWrap` | Gets or sets the wrap. |
...
|--------|---------|-------------|
| AddItem(RadzenCheckBoxListItem<TValue> item) | `void` | Adds the item. |
| RemoveItem(RadzenCheckBoxListItem<TValue> item) | `void` | Removes the item. |
