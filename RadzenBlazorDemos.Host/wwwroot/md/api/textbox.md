...
| MaxLength | `long?` | Gets or sets the maximum number of characters that can be entered in the text box. When set, the browser will prevent users from typing beyond this limit. |
...
| ReadOnly | `bool` | Gets or sets whether the text box is read-only and cannot be edited by the user. When true, the text box displays the value but prevents user input while still allowing selection and copying. |
...
| Trim | `bool` | Gets or sets whether to automatically remove leading and trailing whitespace from the value. When enabled, whitespace is trimmed when the value changes. |
