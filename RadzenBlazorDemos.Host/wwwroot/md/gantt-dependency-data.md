...
Bind dependencies using a separate POCO collection with ID-based references — ideal for relational databases.

Keywords: gantt, dependency, data, binding, database, id, predecessor, successor

> API reference: [RadzenGantt API](https://blazor.radzen.com/api/gantt.md)

## Examples

## Gantt Dependency Data Binding

Bind dependencies using a separate collection of POCO items with ID-based references — ideal for relational database scenarios. Set `DependencyData`, `DependencyFromProperty`, and `DependencyToProperty` instead of wiring up object references manually.
